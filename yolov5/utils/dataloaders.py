...
def gamma_transform_bgr(im: np.ndarray, gamma: float) -> np.ndarray:
    """对 BGR 图像所有通道做 gamma 变换。"""
    if gamma <= 0:
        raise ValueError("preprocess_gamma must be positive")
    im_float = im.astype(np.float32) / 255.0
    im_gamma = np.power(im_float, gamma)
    return (np.clip(im_gamma, 0.0, 1.0) * 255.0).round().astype(np.uint8)


def rgb_to_hsi(rgb: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """把 RGB 图像转成 HSI，其中 H/S/I 都在 0-1 范围。"""
...
def hsi_to_rgb(hue: np.ndarray, saturation: np.ndarray, intensity: np.ndarray) -> np.ndarray:
    """把 0-1 范围的 HSI 通道转回 RGB uint8 图像。"""
...
def equalize_hsi_intensity_bgr(im: np.ndarray) -> np.ndarray:
    """只对 BGR 图像转 HSI 后的 I 通道做全局直方图均衡。"""
    rgb = cv2.cvtColor(im, cv2.COLOR_BGR2RGB)
    hue, saturation, intensity = rgb_to_hsi(rgb)
    intensity_uint8 = (np.clip(intensity, 0.0, 1.0) * 255.0).round().astype(np.uint8)
    equalized_i = cv2.equalizeHist(intensity_uint8).astype(np.float32) / 255.0
    equalized_rgb = hsi_to_rgb(hue, saturation, equalized_i)
    return cv2.cvtColor(equalized_rgb, cv2.COLOR_RGB2BGR)


def equalize_yuv_luminance_bgr(im: np.ndarray) -> np.ndarray:
    """Only equalize the Y (luminance) channel of a BGR image in YUV space."""
    yuv = cv2.cvtColor(im, cv2.COLOR_BGR2YUV)
    yuv[:, :, 0] = cv2.equalizeHist(yuv[:, :, 0])
    return cv2.cvtColor(yuv, cv2.COLOR_YUV2BGR)


def clahe_yuv_luminance_bgr(im: np.ndarray, clip_limit: float = 2.0, tile_size: int = 8) -> np.ndarray:
    """Apply CLAHE to the Y (luminance) channel of a BGR image in YUV space."""
    if clip_limit <= 0:
        raise ValueError("preprocess_clahe_clip_limit must be positive")
    if tile_size <= 0:
        raise ValueError("preprocess_clahe_tile_size must be positive")
    yuv = cv2.cvtColor(im, cv2.COLOR_BGR2YUV)
...
def preprocess_image_for_detect(
    im: np.ndarray,
    mode: str = "none",
    gamma: float = 0.8,
    ...
    """Apply the selected image preprocessing method to a BGR image before inference."""
    if mode == "none":
        return im
    if mode == "gamma":
        return gamma_transform_bgr(im, gamma)
    if mode == "hsi_equalize":
        return equalize_hsi_intensity_bgr(im)
    if mode == "yuv_equalize":
        return equalize_yuv_luminance_bgr(im)
    if mode == "yuv_clahe":
        return clahe_yuv_luminance_bgr(im, clahe_clip_limit, clahe_tile_size)
    if mode == "gamma_hsi_equalize":
        return equalize_hsi_intensity_bgr(gamma_transform_bgr(im, gamma))
    raise ValueError(f"Unsupported detect preprocess mode: {mode}")
...
def create_dataloader(
    ...
    with torch_distributed_zero_first(rank):  # init dataset *.cache only once if DDP
        dataset = LoadImagesAndLabels(
            ...
            preprocess_clahe_clip_limit=preprocess_clahe_clip_limit,
            preprocess_clahe_tile_size=preprocess_clahe_tile_size,
        )
...
class LoadImages:
    ...
    def __init__(
        ...
        if preprocess not in DETECT_PREPROCESS_MODES:
            raise ValueError(f"preprocess must be one of {DETECT_PREPROCESS_MODES}, got {preprocess}")
        ...
        self.preprocess_clahe_clip_limit = preprocess_clahe_clip_limit
        self.preprocess_clahe_tile_size = preprocess_clahe_tile_size
        if any(videos):
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            im0 = preprocess_image_for_detect(
                im0,
                self.preprocess,
                self.preprocess_gamma,
                self.preprocess_clahe_clip_limit,
                self.preprocess_clahe_tile_size,
            )
...
class LoadImagesAndLabels(Dataset):
    ...
    def load_image(self, i):
        ...
        if im is None:  # not cached in RAM
            ...
            im = preprocess_image_for_detect(
                im,
                self.preprocess,
                self.preprocess_gamma,
                self.preprocess_clahe_clip_limit,
